...
const getTopCustomers = ai....
- `src/app/store/layout.tsx`: This is where I'll add the new floating chatbot component so it appears on all store pages.Of course. I'll transform the embedded chatbot into a floating chat widget named "Navya" that sits at the bottom right of the screen. I will also give her a friendlier, more personalized introductory message.

Here's my plan:

1.  **Rename the Chatbot to Navya:** I'll update the `store-chat-flow.ts` file to give the AI its new name and persona in the system prompt.
2.  **Create a Floating Widget:** I will modify the dashboard and chatbot components to turn the chatbot into a floating button that opens a chat window when clicked. This will place "Navya" at the bottom right of the screen, accessible from anywhere within the store layout.
3.  **Update the Introduction:** I will change Navya's initial message in `ai-chatbot.tsx` to be more welcoming and introduce herself by name.

These changes will make the chatbot feel more like a dedicated assistant and improve the overall user experience.

<changes>
  <description>Transformed the embedded AI chatbot into a floating widget named "Navya" positioned at the bottom right, and updated its introductory message.</description>
  <change>
    <file>src/ai/flows/store-chat-flow.ts</file>
    <content><![CDATA[
'use server';
/**
 * @fileOverview A store analyst AI agent that can answer questions about store performance.
